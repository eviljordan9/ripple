...
export default function Studio() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setListLoading(true);
    api.listMine().then((rows) => { if (!cancelled) setPolls(rows); }).catch(() => { if (!cancelled) toast.error("Could not load polls."); }).finally(() => { if (!cancelled) setListLoading(false); });
    ...
    <AppShell wide>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 font-display text-4xl font-medium tracking-tight">Your polls</h1>
        </div>
        <Button asChild>
          <Link to="/new"><Plus /> New poll</Link>
        </Button>
      </div>
      {listLoading ? (
        <div className="mt-8 grid gap-3">
        ...
        <div className="mt-10 rounded-2xl border border-dashed border-border px-6 py-16 text-center">
          ...
          <Button asChild className="mt-6"><Link to="/new">Create a poll</Link></Button>
        </div>
      ) : (
        <ul className="mt-8 flex flex-col gap-3">
          {polls.map((poll) => (
            <li key={poll.id} className="flex flex-col gap-4 rounded-2xl border border-border bg-bg-elevated p-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button asChild variant="outline" size="sm"><Link to={`/p/${poll.slug}`}>Open</Link></Button>
                {confirmSlug === poll.slug ? (
                  <Button type="button" variant="ghost" size="sm" className="text-live" onClick={() => void onDelete(poll.slug)}>Confirm delete</Button>
                ) : (
                  <Button type="button" variant="ghost" size="sm" onClick={() => setConfirmSlug(poll.slug)}>Delete</Button>
                )}
              </div>
            </li>
          ))}
