...
export default function Login() {
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-sm pt-6">
        <p className="text-xs font-medium uppercase tracking-widest text-muted">Host access</p>
        ...
        <button type="button" className="mt-5 text-sm text-muted hover:text-fg" onClick={() => { setMode(mode === "in" ? "up" : "in"); setError(null); }}>
          {mode === "in" ? "Need an account? Create one" : "Already have one? Sign in"}
        </button>
        <p className="mt-10 text-sm text-subtle">
          Just voting?{" "}
          <Link to="/" className="text-muted underline-offset-4 hover:text-fg hover:underline">Go home</Link>
        </p>
      </div>
    </AppShell>
  );
