...
import { parseJoinInput } from "@/lib/clipboard";
...
export default function Home() {
  ...
  function join(e: FormEvent) {
    e.preventDefault();
    const slug = parseJoinInput(code);
    if (!slug) return;
  ...
  return (
    <AppShell wide>
      <section className="max-w-2xl">
        <p className="text-xs font-medium uppercase tracking-widest text-muted">Live audience polling</p>
        ...
        <p className="mt-5 max-w-md text-base leading-relaxed text-muted">
          Create a poll, share the link or a short code, and watch tallies move as people vote. No refresh. No waiting.
        </p>
        ...
        <form onSubmit={join} className="mt-8 max-w-sm">
          <label htmlFor="join-code" className="text-xs font-medium uppercase tracking-widest text-subtle">Have a code?</label>
          <div className="mt-2 flex gap-2">
            <Input id="join-code" value={code} onChange={(e) => setCode(e.target.value)} placeholder="WELCOME" autoComplete="off" className="font-mono tracking-widest uppercase" />
            <Button type="submit" variant="subtle">Join</Button>
          </div>
        </form>
      </section>
      <section className="mt-12 max-w-xl">
        {loading ? (
          ...
          <div className="rounded-2xl border border-border bg-bg-elevated p-8 text-sm text-muted">Demo poll is warming up. Create your own in the meantime.</div>
        )}
      </section>
